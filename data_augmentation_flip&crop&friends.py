...
for SEED in range(1, 11):
    ...
    class AugmentedAllDataset(Dataset):
        def __init__(self, base_subset):
            self.data = []
            color_aug = transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.02)
            to_pil = transforms.ToPILImage()
            to_tensor = transforms.ToTensor()
            normalize = transform_tensor_input

            for img, label in base_subset:
                # Base
                self.data.append((normalize(img), label))

                # Horizontal Flip
                hflipped = transforms.functional.hflip(img)
                self.data.append((normalize(hflipped), label))
                ...
                self.data.append((normalize(cropped), label))

                # Flip + Crop
                cropped_flipped = transforms.functional.hflip(cropped)
                self.data.append((normalize(cropped_flipped), label))

                # Color Jitter
                color_jittered = transforms.Compose([
                    color_aug,
                    to_tensor
                ])(to_pil(img))
                self.data.append((normalize(color_jittered), label))
                ...
                self.data.append((normalize(rotated), label))
