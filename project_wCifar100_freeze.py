...
NUM_EPOCHS = 50
LEARNING_RATE = 1e-3
NUM_TRAIN_SAMPLES = 200 #200 #1000
...
for SEED in range(1, 11):
    ...
    #********Load pretrained weights from CIFAR-100 ******************************
    pretrained_path = os.path.join(os.path.dirname(__file__), "model_cifar100.pt")  # update seedX
    print(pretrained_path)
    state_dict = torch.load(pretrained_path)

    # Remove classifier weights (layer mismatch between 100 classes and 10 classes)
    del state_dict["classifier.4.weight"]
    del state_dict["classifier.4.bias"]

    model.load_state_dict(state_dict, strict=False)
    print("Loaded pretrained CIFAR-100 weights (excluding final layer).")
    # Optional: freeze convolutional base
