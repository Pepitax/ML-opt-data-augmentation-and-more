...
NUM_EPOCHS = 100
LEARNING_RATE = 1e-3
NUM_TRAIN_SAMPLES = 50_000 #200 #1000
...
for SEED in range(1, 2):
    ...
    full_trainset_raw = datasets.CIFAR100(root=DATA_DIR, train=True, download=True, transform=transform_no_norm)
    ...
    #full_trainset = datasets.CIFAR10(root=DATA_DIR, train=True, download=False, transform=transform)
    full_trainset = datasets.CIFAR100(root=DATA_DIR, train=True, download=False, transform=transform)
    ...
    testset = datasets.CIFAR100(root=DATA_DIR, train=False, download=False, transform=transform)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % 1 == 0:
            va = evaluate()
            print(f'Epoch {epoch} | Acc: {va:.4f}')

    acc = evaluate()
    accuracies.append(acc)
    print(f"Final Test Accuracy: {acc:.4f}")

    model_path = os.path.join(os.path.dirname(__file__), f"model_cifar100.pt")
    torch.save(model.state_dict(), model_path)
    print(f"Model for seed {SEED} saved to {model_path}")
